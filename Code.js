...
function buildTutorialPage() {
  ...
  // ── Styles ──────────────────────────────────────────────────
  + '<style>'
  + '.tut-trial{background:#8b0000;color:#fff;font-weight:700;font-size:13px;'
  +   'padding:10px 14px;border-radius:5px;margin-bottom:20px;text-align:center;'
  +   'line-height:1.6;}'
  + '.tut-nav{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px;}'
  + '.tut-nav a{display:inline-block;padding:8px 14px;background:#f0f4f8;'
  +   'color:#1a3a5c;border:1px solid #c8d4e0;border-radius:5px;font-size:13px;'
  +   'font-weight:600;text-decoration:none;}'
  + '.tut-nav a:hover{background:#1a3a5c;color:#fff;}'
  + '.tut-section{margin-bottom:32px;padding-bottom:28px;'
  +   'border-bottom:1px solid #e8edf2;}'
  + '.tut-section:last-of-type{border-bottom:none;}'
  + '.tut-role-badge{display:inline-block;font-size:11px;font-weight:700;'
  +   'padding:2px 10px;border-radius:10px;margin-bottom:10px;'
  +   'text-transform:uppercase;letter-spacing:0.5px;}'
  + '.badge-voter{background:#e1f5ee;color:#085041;}'
  + '.badge-ec{background:#e6f1fb;color:#0c447c;}'
  + '.badge-scrutineer{background:#faeeda;color:#633806;}'
  + '.badge-ro{background:#f0e6fb;color:#4a0c7c;}'
  + '.badge-all{background:#f0f2f5;color:#333;}'
  + '.tut-h{font-size:1.1rem;font-weight:700;color:#1a3a5c;margin:0 0 6px;}'
  + '.tut-intro{font-size:13px;color:#555;line-height:1.7;margin-bottom:14px;}'
  + '.tut-steps{margin:0;padding:0;list-style:none;}'
  + '.tut-steps li{display:flex;gap:12px;align-items:flex-start;'
  ...
  + '.tut-step-num{background:#1a3a5c;color:#fff;font-size:11px;font-weight:700;'
  +   'width:22px;height:22px;border-radius:50%;display:flex;align-items:center;'
  +   'justify-content:center;flex-shrink:0;margin-top:2px;}'
  + '.tut-note{background:#f0f4f8;border-left:3px solid #1a3a5c;padding:10px 13px;'
  +   'border-radius:0 5px 5px 0;font-size:12px;color:#444;line-height:1.7;'
  +   'margin-top:12px;}'
  + '.tut-note.warn{background:#faeeda;border-left-color:#b8922a;color:#633806;}'
  + '.tut-note.good{background:#e1f5ee;border-left-color:#0f6e56;color:#085041;}'
  + '.tut-back{display:block;text-align:center;margin-top:28px;padding:13px;'
  +   'background:#1a3a5c;color:#fff;border-radius:6px;font-size:15px;'
  +   'font-weight:600;text-decoration:none;}'
  + '.tut-back:hover{background:#0f2540;}'
  + '</style>'

  // ── Trial notice ─────────────────────────────────────────────
  + '<div class="tut-trial">'
  + '&#9432; &nbsp;TRIAL ELECTION — This guide describes the live system. '
  + 'Votes cast during the trial do not count toward any official result.'
  + '</div>'

  // ── Page heading ─────────────────────────────────────────────
  + '<h2 style="color:#1a3a5c;margin:0 0 6px;font-size:1.3rem;">How the Election System Works</h2>'
  + '<p style="font-size:13px;color:#777;margin:0 0 20px;line-height:1.6;">'
  + 'A complete guide for all participants. Use the links below to jump to your role, '
  + 'or scroll through the full guide.</p>'

  // ── Role navigation ──────────────────────────────────────────
  + '<div class="tut-nav">'
  + '<a href="#s-login">&#128274; Logging In</a>'
  + '<a href="#s-voter">&#128499; Voter</a>'
  + '<a href="#s-ec">&#128203; EC Officer</a>'
  + '<a href="#s-scrutineer">&#128269; Scrutineer</a>'
  + '<a href="#s-ro">&#9878; Returning Officer</a>'
  + '<a href="#s-trust">&#128274; Trust &amp; Privacy</a>'
  + '</div>'

  // ══════════════════════════════════════════════════════════════
  // SECTION 1 — LOGGING IN (ALL)
  // ══════════════════════════════════════════════════════════════
  + '<div class="tut-section" id="s-login">'
  + '<div class="tut-role-badge badge-all">All users</div>'
  + '<h3 class="tut-h">1. Logging In</h3>'
  + '<p class="tut-intro">Every user — voter, EC Officer, Scrutineer, or Returning Officer — '
  + 'logs in the same way. There are no passwords. Access is verified by a one-time code '
  + 'sent to your registered email address.</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div>Open the election system link. '
  + 'You will see the Login screen.</div></li>'
  + '<li><div class="tut-step-num">2</div><div><strong>Voters:</strong> enter your '
  + 'roll number. <strong>Admins:</strong> use the Admin Login link at the bottom of '
  + 'the login screen and enter your admin ID.</div></li>'
  + '<li><div class="tut-step-num">3</div><div>Click <strong>Send OTP</strong>. '
  + 'A six-digit code will be sent to your registered email within a few seconds.</div></li>'
  + '<li><div class="tut-step-num">4</div><div>Enter the code on screen and click '
  + '<strong>Verify</strong>. You will be taken to your panel.</div></li>'
  + '<li><div class="tut-step-num">5</div><div>The OTP expires in 10 minutes. '
  + 'If it expires, simply request a new one.</div></li>'
  + '</ul>'
  + '<div class="tut-note warn">&#9888; If you do not receive the OTP, check your spam '
  + 'folder. If your email address is not registered correctly, contact the '
  + 'Returning Officer before voting day.</div>'
  + '</div>'

  // ══════════════════════════════════════════════════════════════
  // SECTION 2 — VOTER
  // ══════════════════════════════════════════════════════════════
  + '<div class="tut-section" id="s-voter">'
  + '<div class="tut-role-badge badge-voter">Voter</div>'
  + '<h3 class="tut-h">2. Voter — What You Can Do</h3>'
  + '<p class="tut-intro">Life members on the certified voter roll may nominate candidates, '
  + 'support nominations, and cast their vote. Each action is available only during '
  + 'the relevant phase of the election.</p>'

  + '<p style="font-weight:700;color:#1a3a5c;font-size:13px;margin:14px 0 6px;">Nominations phase</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div>Log in and go to the '
  + '<strong>Nominations</strong> tab in your voter panel.</div></li>'
  + '<li><div class="tut-step-num">2</div><div>To nominate yourself, click '
  + '<strong>Nominate Myself</strong>, select the post you are standing for, '
  + 'and submit. You will receive a confirmation email.</div></li>'
  + '<li><div class="tut-step-num">3</div><div>To nominate another member, enter '
  + 'their roll number and select the post. The nominee will receive an email '
  + 'to accept or decline.</div></li>'
  + '<li><div class="tut-step-num">4</div><div>To second an existing nomination, '
  + 'find it in the nominations list and click <strong>Second this nomination</strong>. '
  + 'Each nomination requires one seconder.</div></li>'
  + '</ul>'
  + '<div class="tut-note">You may withdraw your own nomination before the nominations '
  + 'deadline. Once the deadline passes, withdrawals are at the discretion of the '
  + 'Returning Officer.</div>'

  + '<p style="font-weight:700;color:#1a3a5c;font-size:13px;margin:18px 0 6px;">Voting phase</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div>Log in during the voting window. '
  + 'You will see the <strong>My Ballot</strong> tab automatically.</div></li>'
  + '<li><div class="tut-step-num">2</div><div>The ballot shows all posts with '
  + 'confirmed candidates. Select one candidate per post.</div></li>'
  + '<li><div class="tut-step-num">3</div><div>Review your selections, then click '
  + '<strong>Cast Vote</strong>. You will be asked to confirm.</div></li>'
  + '<li><div class="tut-step-num">4</div><div>Once confirmed, your vote is recorded '
  + 'and <strong>cannot be changed</strong>. You will receive a receipt token by email.</div></li>'
  + '</ul>'
  + '<div class="tut-note good">&#10003; You can only vote once. The system will not '
  + 'allow a second vote from the same roll number.</div>'

  + '<p style="font-weight:700;color:#1a3a5c;font-size:13px;margin:18px 0 6px;">After voting</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div>Keep your receipt token — it is in '
  + 'your confirmation email.</div></li>'
  + '<li><div class="tut-step-num">2</div><div>After results are declared, go to '
  + '<strong>My Receipts</strong> tab and enter your token to verify your vote '
  + 'was counted.</div></li>'
  + '</ul>'
  + '</div>'

  // ══════════════════════════════════════════════════════════════
  // SECTION 3 — EC OFFICER
  // ══════════════════════════════════════════════════════════════
  + '<div class="tut-section" id="s-ec">'
  + '<div class="tut-role-badge badge-ec">EC Officer</div>'
  + '<h3 class="tut-h">3. EC Officer — What You Can Do</h3>'
  + '<p class="tut-intro">EC Officers — the General Secretary, President, and Vice President — '
  + 'have access to the EC Officer panel only during the pre-election period, up to and '
  + 'including the formal handover to the Returning Officer. At handover their accounts '
  + 'are permanently disabled. They have no access to the election system during or '
  + 'after the election.</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div><strong>Overview tab</strong> — '
  + 'shows the current election status, phase, and key dates at a glance.</div></li>'
  + '<li><div class="tut-step-num">2</div><div><strong>Voter Roll tab</strong> — '
  + 'the General Secretary uploads the verified voter roll CSV as part of the '
  + 'handover to the Returning Officer. This is a one-time upload at handover. '
  + 'The RO then publishes this draft for the member objection window, manages '
  + 'any objections, and certifies the final roll. '
  + 'The President and VP have read-only access to this tab.</div></li>'
  + '<li><div class="tut-step-num">3</div><div><strong>Messages tab</strong> — '
  + 'send messages to voters before handover. All messages are logged.</div></li>'
  + '<li><div class="tut-step-num">4</div><div><strong>Landing Page tab</strong> — '
  + 'the General Secretary publishes the voter verification portal link and manages '
  + 'public-facing election information before handover.</div></li>'
  + '<li><div class="tut-step-num">5</div><div><strong>Handover checklist</strong> — '
  + 'the General Secretary completes the EC-to-RO handover checklist in the system. '
  + 'Submitting it triggers a formal handover notification to the Returning Officer '
  + 'and locks EC Officer access permanently.</div></li>'
  + '</ul>'
  + '<div class="tut-note warn">&#9888; EC Officer access ends permanently at handover. '
  + 'This is enforced at two levels — application (accounts disabled) and credential '
  + '(Google account password changed in the presence of Scrutineers). '
  + 'The EC has no access to nominations, votes, tally, or results at any point.</div>'
  + '</div>'

  // ══════════════════════════════════════════════════════════════
  // SECTION 4 — SCRUTINEER
  // ══════════════════════════════════════════════════════════════
  + '<div class="tut-section" id="s-scrutineer">'
  + '<div class="tut-role-badge badge-scrutineer">Scrutineer</div>'
  + '<h3 class="tut-h">4. Scrutineer — What You Can Do</h3>'
  + '<p class="tut-intro">Scrutineers are appointed by the Returning Officer to review '
  + 'nominations during the scrutiny phase. Each nomination is checked against six '
  + 'eligibility criteria before candidates are published.</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div>Log in during the scrutiny phase. '
  + 'You will see the <strong>Scrutiny</strong> tab with all nominations '
  + 'awaiting review.</div></li>'
  + '<li><div class="tut-step-num">2</div><div>Open each nomination and work through '
  + 'the six-item checklist: nomination form complete, proposer eligible, seconder '
  + 'eligible, nominee on voter roll, post exists in election, tenure bar check.</div></li>'
  + '<li><div class="tut-step-num">3</div><div>Mark each item as Pass or Fail. '
  + 'Add notes where needed.</div></li>'
  + '<li><div class="tut-step-num">4</div><div>Submit your recommendation — '
  + '<strong>Approve</strong> or <strong>Reject</strong>. The final decision '
  + 'rests with the Returning Officer.</div></li>'
  + '</ul>'
  + '<div class="tut-note warn">&#9888; Scrutineers can only view nominations — '
  + 'not voter details, vote counts, or any other election data.</div>'
  + '</div>'

  // ══════════════════════════════════════════════════════════════
  // SECTION 5 — RETURNING OFFICER
  // ══════════════════════════════════════════════════════════════
  + '<div class="tut-section" id="s-ro">'
  + '<div class="tut-role-badge badge-ro">Returning Officer</div>'
  + '<h3 class="tut-h">5. Returning Officer — Managing the Election</h3>'
  + '<p class="tut-intro">The Returning Officer (RO) has full control of the election '
  + 'pipeline. The Admin panel has thirteen tabs, each corresponding to a phase '
  + 'or function of the election.</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div><strong>Elections tab</strong> — '
  + 'create the election, set dates, move the election through phases '
  + '(nominations open → scrutiny → candidates published → active → declared).</div></li>'
  + '<li><div class="tut-step-num">2</div><div><strong>Nominations tab</strong> — '
  + 'view all nominations, accept or decline nominee consent, manage the '
  + 'nominations pipeline.</div></li>'
  + '<li><div class="tut-step-num">3</div><div><strong>Scrutiny tab</strong> — '
  + 'assign scrutineers, review their recommendations, approve or reject '
  + 'each nomination.</div></li>'
  + '<li><div class="tut-step-num">4</div><div><strong>Candidates tab</strong> — '
  + 'view the confirmed candidate list before publishing. Once published, '
  + 'candidates are visible to all voters.</div></li>'
  + '<li><div class="tut-step-num">5</div><div><strong>Voters tab</strong> — '
  + 'upload and manage the voter roll CSV.</div></li>'
  + '<li><div class="tut-step-num">6</div><div><strong>Roll Draft tab</strong> — '
  + 'manage the objection window, resolve objections, and certify the '
  + 'final voter roll.</div></li>'
  + '<li><div class="tut-step-num">7</div><div><strong>Tally tab</strong> — '
  + 'available only after voting closes. View vote counts per post and '
  + 'candidate, then formally declare results.</div></li>'
  + '<li><div class="tut-step-num">8</div><div><strong>Handover tab</strong> — '
  + 'complete the post-election handover checklist including GitHub transfer, '
  + 'data archiving, and RO attestation.</div></li>'
  + '<li><div class="tut-step-num">9</div><div><strong>Log tab</strong> — '
  + 'full audit trail of every action taken in the system. '
  + 'Immutable and timestamped.</div></li>'
  + '</ul>'
  + '<div class="tut-note good">&#10003; The RO is the only person who can move '
  + 'the election between phases. Every phase change is logged with a timestamp '
  + 'and the RO\'s identity.</div>'
  + '</div>'

  // ══════════════════════════════════════════════════════════════
  // SECTION 6 — TRUST & PRIVACY
  // ══════════════════════════════════════════════════════════════
  + '<div class="tut-section" id="s-trust">'
  + '<div class="tut-role-badge badge-all">All users</div>'
  + '<h3 class="tut-h">6. Trust &amp; Privacy</h3>'
  + '<p class="tut-intro">The system is designed so that no single person — including '
  + 'the Returning Officer — can link a vote to a voter. This is built into the '
  + 'architecture, not just promised by policy.</p>'
  + '<ul class="tut-steps">'
  + '<li><div class="tut-step-num">1</div><div><strong>Structural anonymity</strong> — '
  + 'votes are stored in a Votes sheet with no voter identity. The record that '
  + 'you voted (but not how) is stored separately in a VotedLog sheet. '
  + 'These two sheets are never joined.</div></li>'
  + '<li><div class="tut-step-num">2</div><div><strong>RO independence</strong> — '
  + 'once the RO is appointed, the EC is locked out of the election system. '
  + 'The RO operates independently until formal handover after declaration.</div></li>'
  + '<li><div class="tut-step-num">3</div><div><strong>Receipt token</strong> — '
  + 'your vote generates an anonymous token. After results are declared you can '
  + 'enter this token to confirm your vote was counted — without revealing '
  + 'what you voted.</div></li>'
  + '<li><div class="tut-step-num">4</div><div><strong>Data stays with OBA</strong> — '
  + 'all election data is stored in the Association\'s own Google account. '
  + 'No third party holds or processes your vote.</div></li>'
  + '<li><div class="tut-step-num">5</div><div><strong>Audit trail</strong> — '
  + 'every administrative action is logged with a timestamp and identity. '
  + 'The log is append-only and cannot be edited.</div></li>'
  + '</ul>'
  + '<div class="tut-note good">&#10003; The source code for this system is publicly '
  + 'available on GitHub. Any member may review it or have it independently audited.'
  + '</div>'
  + '</div>'

  // ── Back to home ─────────────────────────────────────────────
  + '<a class="tut-back" href="' + ScriptApp.getService().getUrl() + '">&#8592; Back to Election Home</a>'
  + roContactFooter();

  return standaloneShell('How It Works — SSKZM OBA Election System', body);
}
