...
var RO_CONTACT_EMAIL  = PropertiesService.getScriptProperties().getProperty('RO_CONTACT_EMAIL') || 'admin@sskzmoba.org';
var ELECTIONS_EMAIL   = 'elections.sskzmoba@gmail.com';
var ELECTIONS_NAME    = 'SSKZM OBA Elections';
var EC_OFFICIAL_EMAIL = 'sskzmobakazak@gmail.com'; // official EC mailbox — CC'd on mass voter notices for visibility
...
var COL = {
  ...
  ELEC_SHOW_ON_LANDING:30, // explicit single-select "this is the headline election on the public Landing Page" flag
...
var COL_AP = {
  ID:        0,  // unique ID
  NAME:      1,  // Appeals Panel member's name
  ROLL:      2,  // Roll No
  BATCH:     3,  // Batch
  EMAIL:     4,  // registered email — where appeal/objection notices go
  IS_CHAIR:  5,  // Boolean — most senior member per SOP 2A.12
  ADDED_AT:  6,  // ISO timestamp
  ADDED_BY:  7   // AdminID who added this contact
};  // 8 cols ✓
...
// Access: RO_ADMIN, DEPUTY_RO (matches recordROAuthorisation — this data only feeds
// the RO's own authorisation-issuing screen; Deputy RO acts with full RO authority
// while activated, per SOP 2A.1).
function getTemAuthorisableActions(token) {
  ...
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO') return { success: false, message: 'Access denied. RO only.' };
  return {
    success:  true,
...
// Formats a Date (or date-parseable value) as a readable IST datetime string
// for email/UI display, e.g. "26 Jul 2026, 8:14 AM IST".
function fmtIST(date) {
  var d = (date instanceof Date) ? date : new Date(date);
  if (isNaN(d.getTime())) return '';
  return Utilities.formatDate(d, 'GMT+5:30', 'd MMM yyyy, h:mm a') + ' IST';
}
...
function getVoterRollRows(electionId) {
  // Only trust the (global, shared) Voters sheet as "certified" if THIS
  // election is the one that certified it — not just because the sheet
  // happens to hold data left over from a different election's certification.
...
function setElectionSchedule(token, electionId, schedData, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'EC_OFFICER') {
    return { success: false, message: 'Access denied.' };
  }
...
function getPublicSchedule() {
  try {
  // Scoped to the same election getPublicElectionStatus is showing as the
  // headline — previously scanned the whole sheet unscoped, which let a
  // different, unrelated election's stale published schedule keep showing.
  var targetElecId = _findCurrentPublicElectionId();
  var schedRows = sheetData(SHEETS.ELECTION_SCHED);
  ...
  for (var s = 0; s < schedRows.length; s++) {
    var r    = schedRows[s];
    if (!targetElecId || r[COL_SCHED.ELEC_ID].toString() !== targetElecId) continue;
...
// ============================================================
// getLandingPageContent — returns landing page content entries.
// No auth required. Pass publicOnly=true to filter to items
// marked "Visible on public landing page" (used by the public
// Landing Page itself); omitted/false returns everything,
// including internal-only items (used by the Settings admin UI).
// ============================================================
function getLandingPageContent(publicOnly) {
  var rows = sheetData(SHEETS.LANDING_CONTENT);
  var items = [];
  for (var i = 0; i < rows.length; i++) {
    if (rows[i][COL_LPC.KEY].toString() === '') continue;
    var isPublic = rows[i][COL_LPC.PUBLIC].toString() === 'true';
...
function setLandingPageContent(token, key, value, type, label, publicVisible, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'EC_OFFICER' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
...
function getAllElections(token) {
  ...
  var elections = rows.map(function(r) {
    return {
      ...
      showOnLandingPage: r[COL.ELEC_SHOW_ON_LANDING] ? r[COL.ELEC_SHOW_ON_LANDING].toString().toLowerCase() === 'true' : false
    };
  });
...
// ============================================================
// setShowOnLandingPage — explicit, single-select flag marking which
// election is the headline one on the public Landing Page. Setting it
// on one election clears it on every other. Passing showOnLandingPage
// = false just clears it (falls back to the auto priority-scan).
// Access: RO_ADMIN, DEPUTY_RO, TEM
// ============================================================
function setShowOnLandingPage(token, electionId, showOnLandingPage, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  var temCheck = requiresTEMAuth(sess, authId, 'setShowOnLandingPage', electionId);
  if (!temCheck.pass) return { success: false, message: temCheck.message };
  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][COL.ELEC_ID].toString() === electionId.toString()) {
      sh.getRange(i + 1, COL.ELEC_SHOW_ON_LANDING + 1).setValue(showOnLandingPage === true);
      found = true;
    } else if (showOnLandingPage === true) {
      // Single-select — clear the flag on every other election.
      sh.getRange(i + 1, COL.ELEC_SHOW_ON_LANDING + 1).setValue(false);
    }
  }
  if (!found) return { success: false, message: 'Election not found.' };

  appendAdminLog(sess.identity, 'landing_page_election_set',
    (showOnLandingPage ? 'Set as' : 'Removed as') + ' the headline election on the public Landing Page.',
    '', electionId);
  return { success: true };
}
...
function createElection(token, data, authId) {
  ...
  row[COL.ELEC_SHOW_ON_LANDING]       = false;
...
function updateElection(token, id, data, authId) {
  ...
  if (data.title !== undefined) {
    var newTitle = data.title.toString().trim();
    if (newTitle === '') return { success: false, message: 'Election title cannot be blank.' };
    sh.getRange(rowIndex, COL.ELEC_TITLE + 1).setValue(newTitle);
  }
...
function recordChecklistItem(token, electionId, itemCode, notes, authId) {
  ...
  // D1 ("Voter roll certified and row count verified") is the one item this
  // checklist can actually verify programmatically instead of trusting a
  // self-attested note — SOP 3.6 requires certification before this point,
  // so don't let the RO tick it if isVoterRollCertified() says otherwise.
  if (itemCode === 'D1' && !isVoterRollCertified(electionId)) {
    return { success: false, message: 'Cannot mark D1 complete — the voter roll for this election has not been certified yet.' };
  }
...
// Returns the subset of the four mandatory posts (President, Vice
// President, General Secretary, Treasurer) with no accepted candidate for
// this election. Shared by the scrutiny→candidates_published warning and
// the candidates_published→active hard block, so the two checks can never
// drift apart on what "vacant" means.
function _getVacantMandatoryPosts(electionId) {
  ...
  for (var mp = 0; mp < MANDATORY_POSTS.length; mp++) {
    if (!filledPosts[MANDATORY_POSTS[mp]]) vacant.push(MANDATORY_POSTS[mp]);
  }
  return vacant;
}
...
function updateElectionStatus(token, electionId, newStatus, overrideNote, authId) {
  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][COL.ELEC_ID].toString() === electionId.toString()) {
      ...
      // Appeals Panel must be constituted and contactable before the final
      // candidate list is published — per SOP 2A.12 the Panel should be
      // constituted simultaneously with Scrutineer appointments, well
      // before this point, not left until rejections/appeals start
      // happening. Trial elections are exempt (isTrial pattern, C.2).
      if (currentStatus === 'scrutiny' && newStatus === 'candidates_published') {
        var isTrialScrPub = rows[i][COL.ELEC_TRIAL].toString() === 'true';
        if (!isTrialScrPub) {
          var panelContactsGate = sheetData(SHEETS.APPEALS_PANEL);
          if (panelContactsGate.length === 0) {
            return {
              success: false,
              message: 'Cannot publish the final candidate list — no Appeals Panel contacts are on record. ' +
                       'The Appeals Panel (SOP 2A.12) must be constituted and its contact details recorded ' +
                       'before scrutiny can conclude.'
            };
          }
        }

        // ── SEMI-BLOCK: mandatory post vacant — warn now, while it's still
        // cheap to fix via Phase 2 extension, rather than only discovering it
        // later at the candidates_published → active gate (by which point
        // getting back to scrutiny needs a manual revert). Overridable —
        // the RO's judgment is final (SOP 2.5) — but requires a conscious
        // confirmation, not a silent pass-through.
        var vacantAtPublish = _getVacantMandatoryPosts(electionId);
        var vacancyConfirmed = overrideNote && overrideNote.toString().indexOf('vacant_mandatory_post_confirmed') === 0;
        if (vacantAtPublish.length > 0 && !vacancyConfirmed) {
          return {
            success: false,
            requiresVacancyConfirm: true,
            vacantPosts: vacantAtPublish,
            message: 'The following mandatory post' + (vacantAtPublish.length > 1 ? 's have' : ' has') +
                     ' no accepted candidate: ' + vacantAtPublish.join(', ') + '. Per SOP 4.9, you should ' +
                     'trigger a Phase 2 nomination extension now, while still in scrutiny — extension is not ' +
                     'possible once the candidate list is published.'
          };
        }
      }
      ...
      if (currentStatus === 'candidates_published' && newStatus === 'active') {
        ...
        var vacantMandatory = _getVacantMandatoryPosts(electionId);
...
// ============================================================
// getVacantMandatoryPosts — read-only check, used by the Elections tab to
// show the "Revert to Scrutiny" card only when actually relevant.
// Access: RO_ADMIN, DEPUTY_RO, TEM
// ============================================================
function getVacantMandatoryPosts(token, electionId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  return { success: true, vacantPosts: _getVacantMandatoryPosts(electionId) };
}

// ============================================================
// revertToScrutinyForVacancy — RO reverts candidates_published → scrutiny
// specifically when a mandatory post has become vacant AFTER the candidate
// list was published (e.g. an appeal upheld, or an acceptance undone) —
// the scrutiny→candidates_published semi-block only catches vacancy at
// publish time, not one arising afterward. Deliberately narrow: this is
// NOT a general-purpose "undo publication" button — it only fires when a
// mandatory post is actually vacant right now, so it can't be used to
// silently reopen scrutiny for unrelated reasons.
// Access: RO_ADMIN, DEPUTY_RO, TEM (AuthID-gated)
// ============================================================
function revertToScrutinyForVacancy(token, electionId, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  var temCheck = requiresTEMAuth(sess, authId, 'revertToScrutinyForVacancy', electionId);
  if (!temCheck.pass) return { success: false, message: temCheck.message };
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    var currentStatus = rows[i][COL.ELEC_STATUS].toString();
    if (currentStatus !== 'candidates_published') {
      return { success: false, message: 'This action is only available while status is candidates_published. ' +
        'Current status: ' + currentStatus + '.' };
    }

    var vacantMandatory = _getVacantMandatoryPosts(electionId);
    if (vacantMandatory.length === 0) {
      return { success: false, message: 'No mandatory post is currently vacant — nothing to revert for.' };
    }

    sh.getRange(i + 1, COL.ELEC_STATUS + 1).setValue('scrutiny');
    appendAdminLog(sess.identity, 'reverted_to_scrutiny_for_vacancy',
      'Election reverted candidates_published → scrutiny — mandatory post(s) vacant: ' +
      vacantMandatory.join(', ') + '. Trigger a Phase 2 extension to seek nominations.',
      'candidates_published', electionId);

    return { success: true, vacantPosts: vacantMandatory };
  }
...
function getVoterRollDraft(token, page, search) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'EC_OFFICER' && sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
...
// ============================================================
function uploadVoterRollDraft(token, rows, authId, electionId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'EC_OFFICER' && sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
...
function getMessages(token) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'EC_OFFICER' && sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO') {
    return { success: false, message: 'Access denied.' };
  }
...
function acknowledgeMessage(token, messageId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO') return { success: false, message: 'Access denied.' };
...
function roContactFooter() {
  return '<p class="ro-contact">For assistance, contact ' + ELECTIONS_NAME + ': '
    + '<a href="mailto:' + ELECTIONS_EMAIL + '">' + ELECTIONS_EMAIL + '</a></p>';
}
...
function buildResultsPage(electionId) {
  ...
  if (posts.length === 0) {
    ...
    for (var i = 0; i < posts.length; i++) {
      ...
      if (post.tieUnresolved) {
        html +=
          '<div style="background:#fff7ed;border:1px solid #fdba74;border-radius:6px;' +
          'padding:8px 12px;margin-bottom:8px;font-size:.8rem;color:#9a3412;">' +
          '⚠ Tied — pending a formal draw of lots (SOP 8.4) to determine the winner.</div>';
      }
      ...
      for (var j = 0; j < cands.length; j++) {
        ...
        var isTied    = cand.tied;
        var rowColor  = isElected ? '#065f46' : (isTied ? '#9a3412' : '#374151');
        var icon      = isElected ? '✅' : (isTied ? '⏳' : '<span style="color:#d1d5db;">○</span>');
        var badge     = isElected
          ...
          : (isTied
            ? ' <span style="font-size:.72rem;background:#fff7ed;color:#9a3412;' +
              'padding:2px 7px;border-radius:8px;font-weight:600;margin-left:4px;">Tied — Pending</span>'
            : '');
        html +=
...
// ============================================================
// isVoterRollCertified — true if the roll has been certified.
// Pass electionId to check that SPECIFIC election's own ELEC_CERTIFIED_AT
// field — each election certifies its own roll snapshot independently
// (confirmed intent: a fresh certification is expected per real election
// cycle, e.g. a byelection re-certifying after membership changes since
// the last one). Certifying one election must never lock another.
// If electionId is omitted (only findVoter()'s login-time lookup does
// this, which has no election context yet to scope to), falls back to
// the previous priority-scan guess across all elections.
// Gates the findVoter() draft fallback and all VoterRollDraft mutations,
// per SOP 3.6 — no changes to the roll after certification, ever.
// ============================================================
function isVoterRollCertified(electionId) {
  var elections = sheetData(SHEETS.ELECTIONS);
  if (electionId) {
    for (var i = 0; i < elections.length; i++) {
      if (elections[i][COL.ELEC_ID].toString() === electionId.toString()) {
        return elections[i][COL.ELEC_CERTIFIED_AT].toString().trim() !== '';
      }
    }
    return false; // election not found — treat as not certified
  }
  // 'declared' deliberately excluded — a concluded election must never be
  // picked as "current" here, or its stale certification status would
  // wrongly govern login/draft-fallback for whatever election comes next.
  var priority = [
...
function findVoter(rollNo) {
  ...
  // Decide which sheet is authoritative FIRST (mirrors getVoterRollRows),
  // then look up the roll number in only that one sheet. Previously this
  // checked Voters unconditionally before ever considering the draft, so
  // someone removed from the current cycle's draft but still lingering in
  // a previous (superseded but not-yet-overwritten) certified Voters sheet
  // would still be found and allowed to log in.
...
function getPublicElectionStatus() {
  try {
    // Explicit RO-set flag takes priority — falls back to the auto
    // priority-scan only when no election has been flagged.
    var best = _getFlaggedElectionRow();
    ...
    // vva_url is sourced solely from getLandingPageContent(true) on the
    // client side now — this function no longer duplicates that read.
    return {
      found:       true,
...
function initSystemBSheets() {
  ...
  var HEADERS = {
    ...
    'Elections': [
      ...
      'CandPubAt','VotesHash','InternalTest','ShowOnLandingPage'
    ],
    'Candidates': [
...
function sendNominationCall(token, electionId, customNote, authId) {
  ...
  var rollDescriptor = vrResult.source === 'certified' ? 'certified' : 'draft';
  ...
  for (var v = 0; v < voterRows.length; v++) {
    ...
    var body =
      ...
      '<p>Nominations are now open for the <strong>' + elecTitle + '</strong>.</p>' +
      '<p>All Life Members on the ' + rollDescriptor + ' voter roll are eligible to nominate, ' +
  ...
  try {
    sendEmailViaSendGrid(EC_OFFICIAL_EMAIL, '[EC COPY] Nomination Call Sent: ' + elecTitle,
      '<p>The nomination call notice was sent to ' + sent + ' voter(s) for <strong>' + elecTitle + '</strong>.' +
      (failed > 0 ? ' ' + failed + ' failed to send.' : '') + '</p>' +
      (customNote ? '<p>Note included: ' + customNote + '</p>' : ''));
  } catch (e) {}
...
function sendVoterRollPublicationNotice(token, electionId, objectionDeadline, authId) {
  ...
  for (var v = 0; v < voterRows.length; v++) {
    ...
    var batch = voterRows[v][COL.VOTER_BATCH].toString().trim();
    if (!email) { failed++; continue; }
    ...
    var body =
      ...
      '<p>Name: <strong>' + name + '</strong><br>Roll No: <strong>' + roll +
      '</strong><br>Batch: <strong>' + batch + '</strong></p>' +
  ...
  try {
    sendEmailViaSendGrid(EC_OFFICIAL_EMAIL, '[EC COPY] Voter Roll Publication Notice Sent: ' + elecTitle,
      '<p>The draft voter roll publication notice was sent to ' + sent + ' voter(s) for <strong>' + elecTitle + '</strong>.' +
      (failed > 0 ? ' ' + failed + ' failed to send.' : '') + '</p>' +
      (objectionDeadline ? '<p>Objection deadline communicated: ' + objectionDeadline + '</p>' : ''));
  } catch (e) {}
...
function sendCandidateListPublishedNotice(token, electionId, authId) {
  ...
  try {
    sendEmailViaSendGrid(EC_OFFICIAL_EMAIL, '[EC COPY] Candidate List Published Notice Sent: ' + elecTitle,
      '<p>The candidate list published notice was sent to ' + sent + ' voter(s) for <strong>' + elecTitle + '</strong>.' +
      (failed > 0 ? ' ' + failed + ' failed to send.' : '') + '</p>');
  } catch (e) {}
...
// ============================================================
// sendVotingOpenNotice — notifies all voters that voting has opened.
// Was a gap — no notice existed for the candidates_published -> active
// transition; the candidate-list-published notice covers the withdrawal
// deadline but never mentions a voting link or date. RO-triggered, not
// automatic, matching every other lifecycle notice in this file.
// Access: RO_ADMIN, TEM (TEM-gated)
// ============================================================
function sendVotingOpenNotice(token, electionId, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') return { success: false, message: 'Access denied.' };
  var temCheck = requiresTEMAuth(sess, authId, 'sendVotingOpenNotice', electionId);
  if (!temCheck.pass) return { success: false, message: temCheck.message };
  ...
  var elecTitle  = elec[COL.ELEC_TITLE].toString();
  var elecStatus = elec[COL.ELEC_STATUS].toString();
  var atOrAfterActive = ['active', 'paused', 'closed', 'declared'];
  if (atOrAfterActive.indexOf(elecStatus) === -1) {
    return { success: false, message: 'Voting open notice can only be sent once voting is active. Current status: ' + elecStatus + '.' };
  }
  ...
  var voteClose    = elec[COL.ELEC_VOTE_CLOSE] ? elec[COL.ELEC_VOTE_CLOSE].toString() : '';
  var voteCloseFmt = voteClose ? formatISTDeadline(voteClose) : '';
  var sent = 0; var failed = 0;
  ...
  for (var v = 0; v < voterRows.length; v++) {
    ...
    var name  = (voterRows[v][COL.VOTER_NAME].toString() + ' ' +
                 voterRows[v][COL.VOTER_SURNAME].toString()).trim();
    if (!email) { failed++; continue; }
    var subject = 'SSKZM OBA — Voting is Now Open: ' + elecTitle;
    var body =
      '<p>Dear ' + (name || 'Member') + ',</p>' +
      '<p>Voting is now open for the <strong>' + elecTitle + '</strong>.</p>' +
      (voteCloseFmt ? '<p><strong>Voting closes:</strong> ' + voteCloseFmt + '</p>' : '') +
      '<p>Please log in to the election portal to cast your vote:</p>' +
      '<p><a href="' + DEPLOY_URL + '">' + DEPLOY_URL + '</a></p>' +
      '<p>For queries, contact: <a href="mailto:' + ELECTIONS_EMAIL + '">' + ELECTIONS_EMAIL + '</a></p>' +
      '<p>SSKZM OBA Elections</p>';
    try { sendEmailViaSendGrid(email, subject, body); sent++; } catch(e) { failed++; }
  ...
  appendAdminLog(sess.identity, 'voting_open_notice_sent',
    'Voting open notice sent to ' + sent + ' voters. Failed: ' + failed + '. Election: ' + electionId,
    '', electionId);

  try {
    sendEmailViaSendGrid(EC_OFFICIAL_EMAIL, '[EC COPY] Voting Open Notice Sent: ' + elecTitle,
      '<p>The voting open notice was sent to ' + sent + ' voter(s) for <strong>' + elecTitle + '</strong>.' +
      (failed > 0 ? ' ' + failed + ' failed to send.' : '') + '</p>');
  } catch (e) {}

  return { success: true, sent: sent, failed: failed,
    message: 'Voting open notice sent to ' + sent + ' voters.' + (failed > 0 ? ' ' + failed + ' failed.' : '') };
}
...
function sendResultsDeclaredNotice(token, electionId, authId) {
  ...
  try {
    sendEmailViaSendGrid(EC_OFFICIAL_EMAIL, '[EC COPY] Results Declared Notice Sent: ' + elecTitle,
      '<p>The results declared notice was sent to ' + sent + ' voter(s) for <strong>' + elecTitle + '</strong>.' +
      (failed > 0 ? ' ' + failed + ' failed to send.' : '') + '</p>');
  } catch (e) {}
...
function withdrawNomination(token, nomId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  var isRO = (sess.role === 'RO_ADMIN' || sess.role === 'DEPUTY_RO');
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    // ── Alert the RO if this withdrawal just vacated a mandatory post ──
    // SOP 4.5 explicitly permits withdrawal up to D+1 after publication —
    // unlike undoAcceptNomination/deleteCandidate, nothing else here would
    // ever tell the RO this happened; they'd only find out by chance next
    // time they opened the Manage Election screen.
    var withdrawnPost   = rows[i][COL.NOM_POST].toString();
    var withdrawnElecId = rows[i][COL.NOM_ELEC_ID].toString();
    var MANDATORY_POSTS_WD = ['President', 'Vice President', 'General Secretary', 'Treasurer'];
    if (status === 'candidates_published' && MANDATORY_POSTS_WD.indexOf(withdrawnPost) !== -1) {
      var stillVacant = _getVacantMandatoryPosts(withdrawnElecId);
      if (stillVacant.indexOf(withdrawnPost) !== -1) {
        var elecRowsWd = sheetData(SHEETS.ELECTIONS);
        var elecTitleWd = withdrawnElecId;
        for (var ew = 0; ew < elecRowsWd.length; ew++) {
          if (elecRowsWd[ew][COL.ELEC_ID].toString() === withdrawnElecId) {
            elecTitleWd = elecRowsWd[ew][COL.ELEC_TITLE].toString(); break;
          }
        }
        var wdSubject = 'SSKZM OBA Election — Mandatory Post Now Vacant: ' + withdrawnPost;
        var wdBody =
          '<p><strong>SSKZM OBA Election Management System</strong></p>' +
          '<p>' + escHtml(rows[i][COL.NOM_CAND_NAME].toString()) + ' has withdrawn their candidature for ' +
          '<strong>' + escHtml(withdrawnPost) + '</strong> in <strong>' + escHtml(elecTitleWd) + '</strong>, ' +
          'a mandatory post, after the candidate list was already published.</p>' +
          '<p>This post now has no accepted candidate. Voting cannot be activated until this is resolved — ' +
          'either revert to scrutiny and trigger a Phase 2 extension, or record a General Body resolution ' +
          'authorising the election to proceed without this post.</p>';
        _getActiveROEmails().forEach(function(ro) {
          try { sendEmailViaSendGrid(ro.email, wdSubject, wdBody); } catch(e) {}
        });
      }
    }
...
// Shared auto-assessment for post_eligibility, used identically by getScrutinyData
// (display) and acceptNomination (the actual gate) so they can never diverge.
...
// VP/Joint Secretary/Treasurer: no additional criteria beyond Life Member status (bylaw).
// Batch Representative: candidate's batch bracket must match the post's named bracket.
// Organising Secretary: candidate's batch must match the election's restricted batch, if any.
// All other posts: remain Pending for manual Scrutineer review.
function _autoAssessPostEligibility(nom) {
  ...
  if (nomPost === 'president') {
    ...
    return {
      result: peResult.eligible ? 'Yes' : 'No',
    ...
    return {
      result: gsResult.eligible ? 'Yes' : 'No',
...
function getScrutinyData(token, nomId) {
  ...
  // Auto-assess post_eligibility via the shared helper (also used by acceptNomination's
  // gate) so the checklist display and the actual acceptance gate can never diverge.
  if (!savedMap['post_eligibility']) {
    var autoPE = _autoAssessPostEligibility(nom);
    if (autoPE.result !== 'Pending') {
      savedMap['post_eligibility'] = {
        checkItem:   'post_eligibility',
        checkResult: autoPE.result,
        notes:       autoPE.notes
      };
    }
  }
...
// ============================================================
// _persistAutoAssessedScrutinyItems — writes a permanent ScrutinyLog row
// for each of the 6 standard checklist items not already saved for this
// nomination, using the exact same auto-assessment logic already used
// for display (getScrutinyData) and the acceptance gate (acceptNomination)
// — so accepting/rejecting a nomination always leaves a complete audit
// trail (SOP 8.6 item 2), regardless of whether the reviewer stepped
// through each checklist button individually beforehand. Never overwrites
// an already-saved (possibly manually overridden) item.
// ============================================================
function _persistAutoAssessedScrutinyItems(nom, adminId) {
  var nomId    = nom[COL.NOM_ID].toString();
  var elecId   = nom[COL.NOM_ELEC_ID].toString();
  var candRoll = nom[COL.NOM_CAND_ROLL].toString();
  var post     = nom[COL.NOM_POST].toString();
  var isBatchRep = post.toLowerCase().indexOf('batch') !== -1;
  ...
  for (var j = 1; j < scData.length; j++) {
    if (scData[j][COL.SCLOG_NOM_ID].toString() === nomId) {
      saved[scData[j][COL.SCLOG_CHECK_ITEM].toString()] = true;
    }
  }

  var toSave = [];

  if (!saved['post_eligibility']) {
    var autoPE = _autoAssessPostEligibility(nom);
    if (autoPE.result !== 'Pending') {
      toSave.push({ item: 'post_eligibility', result: autoPE.result, notes: autoPE.notes });
    }
  }

  if (!saved['tenure_bar']) {
    var t1r = checkT1TenureBar(candRoll, post);
    var t2r = isBatchRep
      ? { eligible: true, reason: 'Batch Representative — T2 consecutive tenure bar does not apply.' }
      : checkTenureBar(candRoll);
    var tenureEligible = t1r.eligible && t2r.eligible;
    var tenureNotes = '';
    if (!t1r.eligible) tenureNotes += '[AUTO — T1 BAR] ' + t1r.reason;
    if (!t2r.eligible) tenureNotes += (tenureNotes ? ' | ' : '') + '[AUTO — T2 BAR] ' + t2r.reason;
    if (tenureEligible) tenureNotes = '[AUTO] ' + t1r.reason + ' | ' + t2r.reason;
    tenureNotes += ' | To override: save a manual Yes/No with explanatory notes.';
    toSave.push({ item: 'tenure_bar', result: tenureEligible ? 'Yes' : 'No', notes: tenureNotes });
  }

  if (!saved['one_post']) {
    toSave.push({ item: 'one_post', result: 'Yes', notes: '' });
  }
  if (!saved['proposer']) {
    toSave.push({ item: 'proposer', result: nom[COL.NOM_PROP_CONFIRMED].toString() === 'true' ? 'Yes' : 'No', notes: '' });
  }
  if (!saved['seconder']) {
    toSave.push({ item: 'seconder', result: nom[COL.NOM_SEC_CONFIRMED].toString() === 'true' ? 'Yes' : 'No', notes: '' });
  }
  if (!saved['consent']) {
    var consentStatus = nom[COL.NOM_CONSENT_STATUS].toString();
    var consentResult = consentStatus === 'accepted' ? 'Yes' : (consentStatus === '' ? 'N/A' : 'No');
    toSave.push({ item: 'consent', result: consentResult, notes: '' });
  }
  ...
  toSave.forEach(function(entry) {
    ...
    newRow[COL.SCLOG_CAND_ROLL]    = candRoll;
    newRow[COL.SCLOG_POST]         = post;
    newRow[COL.SCLOG_CHECK_ITEM]   = entry.item;
    newRow[COL.SCLOG_CHECK_RESULT] = entry.result;
    newRow[COL.SCLOG_NOTES]        = entry.notes;
    newRow[COL.SCLOG_LOGGED_AT]    = ts;
    newRow[COL.SCLOG_LOGGED_BY]    = adminId;
    scSh.appendRow(newRow);
  });
}
...
function acceptNomination(token, nomId, authId) {
  ...
  // post_eligibility: use saved value if present; otherwise auto-assess via the same
  // shared helper getScrutinyData uses, so display and gate can never diverge.
var postEligResult = savedItems['post_eligibility'];
if (!postEligResult) {
  postEligResult = _autoAssessPostEligibility(nom).result;
}
...
if (!tenureBarOk) {
  ...
  _persistAutoAssessedScrutinyItems(nom, sess.identity);
...
function getElectionsForVoter(token, preferredElectionId) {
  ...
  for (var i = 0; i < elections.length; i++) {
    ...
    var trial = elections[i][COL.ELEC_TRIAL].toString() === 'true';
    visible.push({ row: elections[i], p: (p === -1 ? priority.length : p), trial: trial });
  }
  // Real (non-trial) elections always sort ahead of trial ones, regardless
  // of status — a live trial run being 'active' must never outrank or become
  // the default over a real election still at an earlier stage, since a
  // confused voter could otherwise land on the trial ballot by default and
  // believe they've cast their real vote when they haven't.
...
// Returns [{ email, name }] for every currently ACTIVE RO_ADMIN, plus any
// currently-activated DEPUTY_RO — used to notify whoever is actually
// exercising RO authority right now, replacing the old static
// RO_CONTACT_EMAIL script property.
function _getActiveROEmails() {
  var rows = sheetData(SHEETS.ADMINS);
  var result = [];
  for (var i = 0; i < rows.length; i++) {
    var role   = rows[i][COL.ADMIN_ROLE].toString();
    var status = rows[i][COL.ADMIN_STATUS].toString().toUpperCase();
    if (status !== 'ACTIVE') continue;
    if (role === 'RO_ADMIN' ||
        (role === 'DEPUTY_RO' && rows[i][COL.ADMIN_DEPRO_ACTIVE].toString().toLowerCase() === 'true')) {
      result.push({
...
// Formats the active RO(s) as visible contact text for email bodies —
// e.g. "Jane Doe <jane@example.com>" — so Panel members (who have no EMS
// login to look this up themselves) actually know where to send their
// decision, rather than the email just saying "the Returning Officer"
// with no address attached.
function _roContactLine() {
  var ros = _getActiveROEmails();
  if (ros.length === 0) return '';
  return ros.map(function(ro) {
    return escHtml(ro.name) + ' &lt;' + escHtml(ro.email) + '&gt;';
  }).join(' or ');
}

// ============================================================
// getAppealsPanelContacts — list current Appeals Panel contacts
// Access: RO_ADMIN, DEPUTY_RO, TEM
// ============================================================
function getAppealsPanelContacts(token) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  var rows = sheetData(SHEETS.APPEALS_PANEL);
  var contacts = rows.map(function(r) {
    return {
      id:      r[COL_AP.ID]       ? r[COL_AP.ID].toString()       : '',
      name:    r[COL_AP.NAME]     ? r[COL_AP.NAME].toString()     : '',
      roll:    r[COL_AP.ROLL]     ? r[COL_AP.ROLL].toString()     : '',
      batch:   r[COL_AP.BATCH]    ? r[COL_AP.BATCH].toString()    : '',
      email:   r[COL_AP.EMAIL]    ? r[COL_AP.EMAIL].toString()    : '',
      isChair: r[COL_AP.IS_CHAIR] ? r[COL_AP.IS_CHAIR].toString().toLowerCase() === 'true' : false,
      addedAt: r[COL_AP.ADDED_AT] ? r[COL_AP.ADDED_AT].toString() : ''
    };
  });
...
// ============================================================
// addAppealsPanelContact — RO records an Appeals Panel member's contact
// details (SOP 2A.12 — constituted by the EC, drawn from the RO-selection
// panel; not a system login role, offline deliberation only).
// Access: RO_ADMIN, DEPUTY_RO, TEM (AuthID-gated)
// ============================================================
function addAppealsPanelContact(token, name, rollNo, batch, email, isChair, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  var temCheck = requiresTEMAuth(sess, authId, 'addAppealsPanelContact', null);
  if (!temCheck.pass) return { success: false, message: temCheck.message };

  if (!name || !name.toString().trim()) return { success: false, message: 'Name is required.' };
  if (!email || !email.toString().trim()) return { success: false, message: 'Email is required.' };

  var sh = getSheet(SHEETS.APPEALS_PANEL);
  if (!sh) return { success: false, message: 'AppealsPanel sheet not found. Run initSystemBSheets() first.' };

  var id = generateId();
  var newRow = new Array(8).fill('');
  newRow[COL_AP.ID]       = id;
  newRow[COL_AP.NAME]     = name.toString().trim();
  newRow[COL_AP.ROLL]     = rollNo ? rollNo.toString().trim() : '';
  newRow[COL_AP.BATCH]    = batch ? batch.toString().trim() : '';
  newRow[COL_AP.EMAIL]    = email.toString().trim().toLowerCase();
  newRow[COL_AP.IS_CHAIR] = !!isChair;
  newRow[COL_AP.ADDED_AT] = now().toISOString();
  newRow[COL_AP.ADDED_BY] = sess.identity;
  sh.appendRow(newRow);

  appendAdminLog(sess.identity, 'appeals_panel_contact_added',
    'Appeals Panel contact added: ' + name.toString().trim() +
    (isChair ? ' (Chair)' : ''),
    '', email.toString().trim());
...
// ============================================================
// removeAppealsPanelContact — RO removes an Appeals Panel member's contact
// Access: RO_ADMIN, DEPUTY_RO, TEM (AuthID-gated)
// ============================================================
function removeAppealsPanelContact(token, id, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  var temCheck = requiresTEMAuth(sess, authId, 'removeAppealsPanelContact', null);
  if (!temCheck.pass) return { success: false, message: temCheck.message };

  var sh = getSheet(SHEETS.APPEALS_PANEL);
  if (!sh) return { success: false, message: 'AppealsPanel sheet not found.' };
  var rows = sh.getDataRange().getValues();
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][COL_AP.ID].toString() === id.toString()) {
      var removedName = rows[i][COL_AP.NAME].toString();
      sh.deleteRow(i + 1);
      appendAdminLog(sess.identity, 'appeals_panel_contact_removed',
        'Appeals Panel contact removed: ' + removedName, id.toString(), '');
      return { success: true };
    }
  }
...
// ============================================================
function fileAppeal(token, nominationId, appealText, docFilename, docBase64, docMimeType) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  ...
  // ── One open appeal per nomination ───────────────────────────
  // Prevents duplicate rows from double-clicks or retries after a client-side
  // "connection error" that actually succeeded server-side (each duplicate
  // independently counts toward the pending-appeals gate otherwise).
  var existingAplRows = sheetData(SHEETS.APPEALS);
  for (var ex = 0; ex < existingAplRows.length; ex++) {
    var exRow = existingAplRows[ex];
    if (exRow[COL_APL.NOM_ID].toString()      !== nominationId.toString()) continue;
    if (exRow[COL_APL.APPEAL_TYPE].toString() !== 'rejection_appeal') continue;
    var exStatus = exRow[COL_APL.STATUS].toString();
    if (exStatus === 'filed' || exStatus === 'under_review') {
      return { success: false, message: 'You already have an open appeal against this rejection (Ref: ' +
        exRow[COL_APL.ID].toString() + '). Please wait for a decision before filing another.' };
    }
  }
  ...
  // ── Upload supporting document inline, BEFORE composing the Panel email,
  // so its link is ready to embed. Non-fatal — the appeal itself is already
  // filed regardless of whether this succeeds.
  var docDriveUrl = '';
  if (docFilename && docBase64) {
    try {
      var docResult = _storeAppealSupportingDoc(nom[COL.NOM_ELEC_ID].toString(), elecTitle, id,
        sess.identity, sess.role, docFilename, docBase64, docMimeType);
      docDriveUrl = docResult.driveUrl;
    } catch(e) {
      // Filing still succeeds even if the document upload fails.
    }
  }

  // ── Notify Appeals Panel immediately ─────────────────────────
  var panelContacts = sheetData(SHEETS.APPEALS_PANEL);
  if (panelContacts.length > 0) {
    var panelSubject = 'SSKZM OBA Election — Appeal Filed: ' + nom[COL.NOM_POST].toString();
    var roContactLineAppeal = _roContactLine();
    var panelBody =
      ...
      (docDriveUrl
        ? '<p style="font-weight:600;color:#1a3a5c;">Supporting Document:</p>' +
          '<p><a href="' + docDriveUrl + '" style="color:#1d4ed8;">📎 ' + escHtml(docFilename) + '</a> ' +
          '<span style="color:#888;font-size:.82rem;">(opens in Google Drive — no login to this system required)</span></p>'
        : '') +
      '<p>Please deliberate and communicate your decision to the Returning Officer' +
      (roContactLineAppeal ? ' at ' + roContactLineAppeal : '') +
      '. The Returning Officer will record the decision in the Election Management System.</p>' +
      '<p style="color:#888;font-size:.82rem;">SSKZM OBA Election Management System</p>' +
      '</div></div>';
    panelContacts.forEach(function(c) {
      var email = c[COL_AP.EMAIL] ? c[COL_AP.EMAIL].toString().trim() : '';
      if (email) { try { sendEmailViaSendGrid(email, panelSubject, panelBody); } catch(e) {} }
    });

    // ── Send copy to EC (if a contact is on record for this election) ──
    var ecContactAppeal = elecRec && elecRec[COL.ELEC_EC_CONTACT] ? elecRec[COL.ELEC_EC_CONTACT].toString().trim() : '';
    if (ecContactAppeal) {
      try { sendEmailViaSendGrid(ecContactAppeal, '[EC COPY] ' + panelSubject, panelBody); } catch(e) {}
    }
  }
  // ─────────────────────────────────────────────────────────────

  return { success: true, appealId: id, docUploaded: !!docDriveUrl };
}
...
// ============================================================
function fileObjection(token, nominationId, groundsType, objectionText, docFilename, docBase64, docMimeType) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  ...
  // ── Upload supporting document inline (non-fatal if it fails) — its
  // Drive link also carries through into the consolidated Panel summary
  // later, since objections aren't notified to the Panel individually.
  var docDriveUrl = '';
  if (docFilename && docBase64) {
    try {
      var docResult = _storeAppealSupportingDoc(nom[COL.NOM_ELEC_ID].toString(), elecTitle, id,
  ...
  var activeROs = _getActiveROEmails();
  if (activeROs.length > 0) {
    ...
    var roBody =
      ...
      '</table>' +
      (docDriveUrl
        ? '<p><a href="' + docDriveUrl + '" style="color:#1d4ed8;">📎 ' + escHtml(docFilename) + '</a></p>'
        : '') +
      '<p style="color:#888;font-size:.82rem;">SSKZM OBA Election Management System</p>' +
      '</div></div>';
    activeROs.forEach(function(ro) {
      try { sendEmailViaSendGrid(ro.email, roSubject, roBody); } catch(e) {}
    });
  }

  return { success: true, objectionId: id, docUploaded: !!docDriveUrl };
}
...
function sendConsolidatedObjectionSummary(token, electionId, authId) {
  ...
  // ── Fetch election record (title + deadline display below) ────
  var elecRows = sheetData(SHEETS.ELECTIONS);
  var elecRec = null;
  for (var e = 0; e < elecRows.length; e++) {
    if (elecRows[e][COL.ELEC_ID].toString() === electionId.toString()) {
  ...
  if (!elecRec) return { success: false, message: 'Election not found.' };
  var candPubAt = elecRec[COL.ELEC_CAND_PUB_AT] ? elecRec[COL.ELEC_CAND_PUB_AT].toString() : '';

  // ── DocStore lookup (docId → {filename, url}), for resolving DOC_LINKS ──
  var docLookup = {};
  sheetData(SHEETS.DOC_STORE).forEach(function(d) {
    docLookup[d[COL.DOC_ID].toString()] = {
      filename: d[COL.DOC_FILENAME].toString(),
      url:      d[COL.DOC_GDRIVE_URL].toString()
    };
  });
  ...
  for (var i = 0; i < aplRows.length; i++) {
    ...
    var docLinksRaw = r[COL_APL.DOC_LINKS] ? r[COL_APL.DOC_LINKS].toString().trim() : '';
    var docs = docLinksRaw ? docLinksRaw.split(',').map(function(docId) { return docLookup[docId.trim()]; }).filter(Boolean) : [];
  ...
  var panelDeadline = candPubAt
    ...
    : 'as soon as possible';
  var objFilingWindowClosed = candPubAt
    ? now().getTime() > (new Date(candPubAt).getTime() + (48 * 60 * 60 * 1000))
    : false;
  var windowStatusLine = objFilingWindowClosed
    ? 'The 48-hour objection window has closed for <strong>' + escHtml(elecTitle) + '</strong>.'
    : 'The 48-hour objection window for <strong>' + escHtml(elecTitle) + '</strong> is still open — ' +
      'this summary is being sent early so the Panel has as much time as possible to deliberate. ' +
      'A follow-up summary will be sent if further objections are filed before the window closes.';
  ...
  nomIds.forEach(function(nomId) {
    ...
    g.objections.forEach(function(obj, idx) {
      var docsHtml = (obj.docs && obj.docs.length > 0)
        ? '<p style="margin:6px 0 0;">' + obj.docs.map(function(d) {
            return '<a href="' + d.url + '" style="color:#1d4ed8;">📎 ' + escHtml(d.filename) + '</a>';
          }).join(' &nbsp;·&nbsp; ') + '</p>'
        : '';
      candidateBlocks +=
  ...
  var subject = 'SSKZM OBA Election — ' + (objFilingWindowClosed ? 'Objection Window Closed' : 'Objection Summary (Early)') +
    ': ' + nomIds.length + ' candidate(s) with objection(s)';
  var body =
    ...
    '</div>' +
    '<div style="padding:24px;border:1px solid #e0e0e0;border-top:none;">' +
    '<p>' + windowStatusLine + '</p>' +
    '<p>The following objections were received and require the attention of the Appeals Panel. ' +
    'Please deliberate and communicate your decisions to the Returning Officer by <strong>' + panelDeadline + '</strong>. ' +
    'The sooner you are able to respond, the sooner the election can proceed to the next stage — ' +
    'please do not wait until the deadline if a decision can be reached earlier.</p>' +
    candidateBlocks +
    ...
    '<strong>Please send a single, joint reply reflecting the Panel\'s collective decision on each candidate — ' +
    'not separate individual responses from each Panel member.</strong> ' +
    'Communicate this consolidated decision with reasons to the Returning Officer' +
    (_roContactLine() ? ' at ' + _roContactLine() : '') +
    ', who will record it in the system.</p>' +
    '<p style="color:#888;font-size:.82rem;">SSKZM OBA Election Management System</p>' +
    '</div></div>';

  // ── Send to Appeals Panel ────────────────────────────────────
  var panelContactsSummary = sheetData(SHEETS.APPEALS_PANEL);
  var sent = 0;
  panelContactsSummary.forEach(function(c) {
    var email = c[COL_AP.EMAIL] ? c[COL_AP.EMAIL].toString().trim() : '';
  ...
  // ── Send copy to RO ──────────────────────────────────────────
  _getActiveROEmails().forEach(function(ro) {
    try { sendEmailViaSendGrid(ro.email, '[RO COPY] ' + subject, body); } catch(e) {}
  });

  // ── Send copy to EC (if a contact is on record for this election) ────
  var ecContactSummary = elecRec[COL.ELEC_EC_CONTACT] ? elecRec[COL.ELEC_EC_CONTACT].toString().trim() : '';
  if (ecContactSummary) {
    try { sendEmailViaSendGrid(ecContactSummary, '[EC COPY] ' + subject, body); } catch(e) {}
  }
...
// ============================================================
// _storeAppealSupportingDoc — shared upload logic for a supporting document
// against an Appeals-sheet record (appeal or objection). Used both by the
// standalone storeAppealDocument RPC and inline by fileAppeal/fileObjection
// so a document attached at filing time is already uploaded — with its
// Drive link ready to embed — before the notification email is composed.
// The Drive file is set to ANYONE_WITH_LINK/VIEW specifically so Appeals
// Panel members (who have no EMS login at all) can open it straight from
// the email with no authentication against this system required.
// Throws on failure — callers decide whether that should be fatal.
// ============================================================
function _storeAppealSupportingDoc(electionId, elecTitle, appealId, uploaderRoll, uploaderRole, filename, base64Data, mimeType) {
  ...
  newRow[COL.DOC_UPLOADER_ROLE] = uploaderRole;
  ...
  // Update DOC_LINKS field on the appeal/objection row
  ...
  appendAdminLog(uploaderRoll, 'appeal_document_uploaded',
    'Supporting document uploaded for ' + appealId + ' | File: ' + filename + ' | DocID: ' + docId,
    '', electionId);
...
// AFTER filing (e.g. adding a second document later). For the primary
// document-at-filing-time flow, see fileAppeal/fileObjection, which upload
// inline so the Panel/RO notification email already has the link.
...
function storeAppealDocument(token, electionId, appealId, filename, base64Data, mimeType) {
  ...
  // Verify the appeal/objection belongs to this voter (candidate for
  // appeals, objector for nomination objections)
  var aplRows = sheetData(SHEETS.APPEALS);
  var apl = null;
  for (var a = 0; a < aplRows.length; a++) {
  ...
  var isObjectionDoc = apl[COL_APL.APPEAL_TYPE] && apl[COL_APL.APPEAL_TYPE].toString() === 'nomination_objection';
  var ownerRoll = isObjectionDoc ? apl[COL_APL.OBJECTOR_ROLL].toString() : apl[COL_APL.CAND_ROLL].toString();
  if (ownerRoll !== sess.identity.toString()) {
    return { success: false, message: 'Access denied — this record does not belong to you.' };
  }
  ...
  try {
    var result = _storeAppealSupportingDoc(electionId, elecTitle, appealId, sess.identity, sess.role, filename, base64Data, mimeType);
    return { success: true, docId: result.docId, driveUrl: result.driveUrl };
  } catch(err) {
...
// ============================================================
// getMyObjections — voter-facing: objections THIS member has personally
// filed against other candidates (as distinct from My Nominations, which
// only covers appeals against their OWN rejection). No persistent tracking
// existed for the objector side before this — filing only returned a
// one-time toast with the reference ID.
// Access: any authenticated session (VOTER)
// ============================================================
function getMyObjections(token, electionId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  ...
  for (var i = 0; i < rows.length; i++) {
    var r = rows[i];
    if (r[COL_APL.APPEAL_TYPE].toString() !== 'nomination_objection') continue;
    if (r[COL_APL.OBJECTOR_ROLL].toString() !== sess.identity.toString()) continue;
    if (electionId && r[COL_APL.ELEC_ID].toString() !== electionId.toString()) continue;
    objections.push({
      id:         r[COL_APL.ID].toString(),
      candName:   r[COL_APL.CAND_NAME].toString(),
      ...
      appealText: r[COL_APL.APPEAL_TEXT].toString(),
      status:     r[COL_APL.STATUS].toString(),
      decision:   r[COL_APL.DECISION] ? r[COL_APL.DECISION].toString() : '',
      decidedAt:  r[COL_APL.DECIDED_AT] ? r[COL_APL.DECIDED_AT].toString() : ''
    });
  }

  objections.sort(function(a, b) {
    return new Date(b.filedAt) - new Date(a.filedAt);
  });
...
// ============================================================
// markAppealDuplicate — RO closes a duplicate appeal/objection row
// (e.g. from a retry after a client-side "connection error" that actually
// succeeded server-side). Distinct from updateAppealDecision: no candidate
// email, no Panel involvement — this is administrative record-keeping, not
// a ruling on the merits. Stops the duplicate counting toward the
// pending-appeals gate.
// Access: RO_ADMIN, DEPUTY_RO, TEM (AuthID-gated)
// ============================================================
function markAppealDuplicate(token, appealId, note, authId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
  }
  var temCheck = requiresTEMAuth(sess, authId, 'markAppealDuplicate', null);
  if (!temCheck.pass) return { success: false, message: temCheck.message };
  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][COL_APL.ID].toString() === appealId.toString()) {
      var priorStatus = rows[i][COL_APL.STATUS].toString();
      sh.getRange(i + 1, COL_APL.STATUS      + 1).setValue('duplicate');
      sh.getRange(i + 1, COL_APL.RO_NOTES    + 1).setValue(note || 'Marked as duplicate.');
      sh.getRange(i + 1, COL_APL.DECIDED_AT  + 1).setValue(new Date());
      sh.getRange(i + 1, COL_APL.DECIDED_BY  + 1).setValue(sess.identity);
      appendAdminLog(sess.identity, 'appeal_marked_duplicate',
        appealId + ' marked duplicate' + (note ? ' | ' + note : ''),
...
// ============================================================
// _computeElectionResults(elec) — shared tally + tie-resolution core.
// Previously duplicated near-identically in getPublicResults and
...
function _computeElectionResults(elec) {
  ...
  return posts.map(function(postName) {
    var group   = postMap[postName];
    var seats   = group.seatCount || 1;
    var nota    = postNota[postName] || 0;
...
// ============================================================
// _findResultsElection(electionId, excludeInternalTest) — shared election
// lookup for the two results endpoints below: use the given ID if
// provided, else the most recently created 'declared' election.
// ============================================================
function _findResultsElection(electionId) {
  var elecRows = sheetData(SHEETS.ELECTIONS);
  var elec = null;
  if (electionId) {
    for (var i = 0; i < elecRows.length; i++) {
    ...
    // Internal-test scratch elections must never be reachable through
    // either results endpoint, even by direct/guessed electionId.
    if (elec && elec[COL.ELEC_INTERNAL_TEST].toString().toLowerCase() === 'true') elec = null;
  } else {
    for (var i = 0; i < elecRows.length; i++) {
      if (elecRows[i][COL.ELEC_INTERNAL_TEST].toString().toLowerCase() === 'true') continue;
      if (elecRows[i][COL.ELEC_STATUS].toString() === 'declared') {
        if (!elec || elecRows[i][COL.ELEC_CREATED_AT] > elec[COL.ELEC_CREATED_AT]) elec = elecRows[i];
      }
    }
  }
  return elec;
...
// ============================================================
// getDeclaredResults — results for declared election, for a logged-in
// voter (My Receipts → View Full Results).
// Access: any authenticated session
// ============================================================
function getDeclaredResults(token, electionId) {
  ...
  var elec = _findResultsElection(electionId);
  ...
  return {
    ...
    electionTitle: elec[COL.ELEC_TITLE].toString(),
    resultVis:     'full_tally',
    posts:         _computeElectionResults(elec)
  };
}
...
// ============================================================
// _getFlaggedElectionRow — returns the full row of whichever election
// has the explicit ELEC_SHOW_ON_LANDING flag set (never an internal-test
// election, even if somehow flagged), or null if none is flagged.
// ============================================================
function _getFlaggedElectionRow() {
  var elections = sheetData(SHEETS.ELECTIONS);
  for (var i = 0; i < elections.length; i++) {
    if (elections[i][COL.ELEC_INTERNAL_TEST].toString().toLowerCase() === 'true') continue;
    if (elections[i][COL.ELEC_SHOW_ON_LANDING] &&
        elections[i][COL.ELEC_SHOW_ON_LANDING].toString().toLowerCase() === 'true') {
      return elections[i];
    }
  }
  return null;
...
// ============================================================
// Mirrors getPublicElectionStatus's selection logic (flagged election
// first, falling back to the priority scan) to pick the same "current"
// election - kept separate rather than refactoring that function, since
// the roll page needs an electionId (for the schedule lookup) that roll
// data itself doesn't carry.
// ============================================================
function _findCurrentPublicElectionId() {
  var flagged = _getFlaggedElectionRow();
  if (flagged) return flagged[COL.ELEC_ID].toString();
...
function purgeTrialData(token, electionId, confirmPhrase, authId) {
  ...
  var counts = {};
  // elecCol is each sheet's ElectionId column index — not uniformly 1
  // (e.g. NomQueries stores it at index 2). Scoping strictly to this
  // electionId is essential: several of these sheets (Votes/VotedLog,
  // PreSecChecklist, DocStore, NomQueries, Observations, Messages) are
  // shared across all elections, so an unfiltered wipe here would destroy
  // real data from any other election sharing the same sheet.
  var sheetsToPurge = [
    { key: 'Candidates',      name: SHEETS.CANDIDATES,       elecCol: 1 },
    { key: 'Votes',           name: SHEETS.VOTES,            elecCol: 1 },
    { key: 'VotedLog',        name: SHEETS.VOTED_LOG,        elecCol: 1 },
    { key: 'Nominations',     name: SHEETS.NOMINATIONS,      elecCol: 1 },
    { key: 'ScrutinyLog',     name: SHEETS.SCRUTINY_LOG,     elecCol: 1 },
    { key: 'Complaints',      name: SHEETS.COMPLAINTS,       elecCol: 1 },
    { key: 'Appeals',         name: SHEETS.APPEALS,          elecCol: 1 },
    { key: 'PreSecChecklist', name: SHEETS.PRESEC_CHECKLIST, elecCol: COL_PRESEC.ELEC_ID },
    { key: 'DocStore',        name: SHEETS.DOC_STORE,        elecCol: COL.DOC_ELEC_ID },
    { key: 'NomQueries',      name: SHEETS.NOM_QUERIES,      elecCol: COL.QRY_ELEC_ID },
    { key: 'Observations',    name: SHEETS.OBSERVATIONS,     elecCol: COL_OBS.ELEC_ID },
    { key: 'Messages',        name: SHEETS.MESSAGES,         elecCol: COL_MSG.ELEC_ID }
  ];
  ...
  for (var s = 0; s < sheetsToPurge.length; s++) {
    ...
    var data = sh.getDataRange().getValues();
    var rowsToDelete = [];
    var elecCol = sheetsToPurge[s].elecCol;
    for (var r = 1; r < data.length; r++) {
      var rowElecId = data[r][elecCol] ? data[r][elecCol].toString() : '';
...
// ============================================================
function updateObjectionStatus(token, rollNo, status, notes, authId, electionId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') return { success: false, message: 'Access denied.' };
...
// Access: RO_ADMIN, TEM (with AuthID)
// ============================================================
function addVoterToDraft(token, rollNo, name, surname, batch, email, notes, authId, electionId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
...
// Access: RO_ADMIN, TEM (with AuthID)
// ============================================================
function updateVoterDraftRow(token, rollNo, name, surname, batch, email, authId, electionId) {
  var sess = getSession(token);
  if (!sess) return { success: false, message: 'Session expired. Please log in again.' };
  if (sess.role !== 'RO_ADMIN' && sess.role !== 'DEPUTY_RO' && sess.role !== 'TEM') {
    return { success: false, message: 'Access denied.' };
...
function _findRecordedDrawWinner(electionId, postName) {
  ...
  for (var i = 0; i < logRows.length; i++) {
    var action = logRows[i][COL.ALOG_ACTION_TYPE].toString();
    var nv = logRows[i][COL.ALOG_NEW_VALUE].toString();
    if (nv !== electionId.toString()) continue;
    ...
    if (action === 'draw_of_lots_conducted') {
      ...
      var did = logRows[i][COL.ALOG_OLD_VALUE].toString();
      if (inSystemDraws[did]) inSystemDraws[did].confirmed++;
  ...
  // Among all fully-witnessed in-system draws for this post, use the most
  // recently CONDUCTED one — a drawId embeds its own conduct timestamp
  // ("DRAW-<ms epoch>"), so a fresh redo always takes priority over an
  // older, stale resolved draw for the same post instead of whichever
  // happened to be encountered first.
  var latestDrawId = null, latestTs = -1;
  for (var d in inSystemDraws) {
    if (inSystemDraws[d].confirmed < 2) continue;
...
function getDrawOfLotsRecords(token, electionId) {
  ...
  var list = Object.keys(draws).map(function(k) { return draws[k]; })
    .sort(function(a, b) { return (a.conductedAtRaw < b.conductedAtRaw) ? 1 : -1; });
...
function _buildElectionRecordHtml(record, generatedBy) {
  ...
  html += '<h2>11. Known Gaps in This Record</h2>' +
    ...
    '<div class="note" style="margin-top:6px;">Handover checklist restructured: ' + escHtml(record.notes.handoverChecklistRestructured) + '</div>';
